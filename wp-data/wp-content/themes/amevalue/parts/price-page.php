<section class="quiz">
  <div class="quiz__wrapper">
    ...
    $slides_title = isset($slides['slides_title'])
      ? $slides['slides_title']
      : '';
    $proposal_summary = isset($slides['proposal_summary'])
      ? $slides['proposal_summary']
      : 'Thanks for your answers. The estimated cost per agent, relative to your request is:';
    $slides_form_title = isset($slides['slides_form_title'])
      ? $slides['slides_form_title']
      : '';
    ?>
<!-- Global header with page title -->
<div class="quiz__header">
  <h1 class="quiz__header-title">
    <span class="quiz__header-title-text"><?php echo esc_html(
      $page_title
    ); ?></span>
  </h1>
</div>
...
<!-- Quiz Form -->
<form id="quizForm" class="quiz__form" data-form-action="<?php echo admin_url(
  'admin-ajax.php'
); ?>">
  <!-- Top Info Section -->
      ...
      <div class="quiz__info-container">
        <div class="quiz__info-container-left">
          ...
          <?php if (!empty($slides_title)): ?>
            <span class="quiz__info-container-left-text"><?php echo esc_html(
              $slides_title
            ); ?></span>
          <?php endif; ?>
        </div>
       ...
       <?php if (!empty($slides_form_title)): ?>
        <div class="quiz__info-container--last-slide-text"><?php echo esc_html(
          $slides_form_title
        ); ?></div>
      <?php endif; ?>
      ...
      <?php if ($slides && !empty($slides['slide_item'])): ?>
        <?php foreach ($slides['slide_item'] as $slide):
          ...
          <div class="quiz__slide">
            <div class="quiz__slide-title"><?php echo esc_html(
              $question_title
            ); ?></div>
              ...
              <div class="quiz__checkbox-list">
                <?php foreach ($slide['slide_checkbox'] as $checkbox):
                  ...
                  <label class="quiz__checkbox">
                    <input class="quiz__checkbox-input" type="checkbox" name="<?php echo esc_attr(
                      $input_name
                    ); ?>[]" value="<?php echo esc_attr($label); ?>" />
                    <span class="quiz__checkbox-box"></span>
                    <span class="quiz__checkbox-label"><?php echo esc_html(
                      $label
                    ); ?></span>
                  </label>
                <?php
              ...
              <div class="quiz__radio-list">
                <?php foreach ($slide['slide_radio'] as $radio):
                  ...
                  <label class="quiz__radio">
                    <input class="quiz__radio-input" type="radio" name="<?php echo esc_attr(
                      $input_name
                    ); ?>" value="<?php echo esc_attr($label); ?>" />
                    <span class="quiz__radio-box"></span>
                    <span class="quiz__radio-label"><?php echo esc_html(
                      $label
                    ); ?></span>
                  </label>
                <?php
