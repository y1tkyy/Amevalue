...
class Custom_Walker_Nav_Menu extends Walker_Nav_Menu
{
    ...
    {
        $class_names = !empty($item->classes)
            ? implode(" ", $item->classes)
            : "";
        $output .=
            '<li class="header__list-item ' . esc_attr($class_names) . '">';
        $output .=
...
function theme_enqueue_assets() {
    ...
    if ( is_page("price") ) {
        wp_enqueue_style(
            "price-style",
            get_template_directory_uri() . "/styles/price.css",
            [],
            "1.0.0",
            "all"
        );
        wp_enqueue_script(
            "price-scripts",
            get_template_directory_uri() . "/js/price-script.js",
            [],
            "1.0.0",
            true
        );
